  ...
  const lineClick = () => {
    liff.init({ liffId: LINEID as string }) // LIFF IDをセットする
      .then(() => {
        if (!liff.isLoggedIn()) {
          // liff.login({}) // ログインしていなければ最初にログインする
          router.push('https://access.line.me/oauth2/v2.1/authorize?app_id=1656149559-xXM4l4Gp&client_id=1656149559&scope=chat_message.write+openid+profile&state=MTSFhIGGxsff&bot_prompt=aggressive&response_type=code&code_challenge_method=S256&code_challenge=Hx-YFyPAvO9ZQIg5pQpaGQuMChsOE11Raf_3DHDGFgY&liff_sdk_version=2.11.1&type=L&redirect_uri=https://redux-next.vercel.app/PageA');
        } else {
          onload()
        }
      })
